...
class helper(helper_def):
	...
	def get_dept_url_list(self):
		...
		dept_list = soup.find(id=u'tabDepartment')
		page_links = dept_list.find_all(u'a')
		...
		for link in page_links:
			...
			print(dept_link)
			match = re.search( '&hs_dept_id=(\d+)', dept_link.strip())
			dept_cd = ''
			if match:
...
#python
import wel_lib as myModule
x = myModule.helper('./config.ini', 'en')
x.get_dept_url_list()
x.get_dept_init_page()
